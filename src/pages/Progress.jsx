...
function LineChart({ points, labels = null, colour = '#006D5B' }) {
  const w = 300, h = 260, padL = 50, padR = 22, padT = 22, padB = 24
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{ height: h }}>
      ...
      {yTicks.map(pct => {
        const y = toY(pct)
        return (
          <text key={pct} x={padL - 6} y={y + 4} fontSize="12" fill="#000000" textAnchor="end" fontWeight="600">{pct}</text>
        )
      })}
      ...
      {xs.map((x, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x} y={y - 12} fontSize="11" fill="#000000" textAnchor="middle" fontWeight="800">
              {points[i]}%
            </text>
          </g>
        )
      ...
      <text
        x={9} y={padT + innerH / 2}
        fontSize="8" fill="#374151" textAnchor="middle" fontWeight="600"
        transform={`rotate(-90, 9, ${padT + innerH / 2})`}
      >Score %</text>

      {labels && labels.map((label, i) => {
        if (labels.length > 4 && i % 2 !== 0) return null
        return (
          <text key={i} x={xs[i]} y={h - 2} fontSize="12" fill="#000000" textAnchor="middle" fontWeight="600">
            {label}
          </text>
        )
      })}
...
function barColour(pct)    { return pct >= 70 ? 'bg-[#006D5B]' : pct >= 50 ? 'bg-amber-400' : 'bg-red-400' }
function headerColour(pct) { return pct >= 70 ? 'text-[#006D5B]' : pct >= 50 ? 'text-amber-600' : 'text-red-500' }

function isFocusSub(pct) { return pct < 50 }
...
function Accordion({ topic }) {
  ...
  return (
    <div className="border border-blue-300 rounded-2xl overflow-hidden mb-3">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-4 py-3.5 bg-blue-100 hover:bg-blue-200 transition-colors"
      >
      ...
      {open && (
        <div className="px-4 pb-4 pt-1 bg-blue-100 border-t border-blue-200">
          {topic.subs.map(s => (
            <div key={s.name} className="mb-3">
              <div className="flex justify-between items-center mb-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {isFocusSub(s.pct) && (
                    <span className="text-[10px] font-black rounded-full px-1.5 py-0.5 text-red-700 bg-red-50 border border-red-200">
                      Focus
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                  ...
                  <Link to={`/practice?topic=${topic.id}`}>
                    <button className="text-[10px] font-black rounded-full px-2 py-0.5 transition-colors text-gray-700 bg-gray-100 border border-gray-300 hover:bg-gray-200">
                      Practice →
                    </button>
                  </Link>
  ...
  if (PRACTICE_EMPTY) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <Link to="/practice">
          <button className="px-8 py-3.5 rounded-xl bg-[#006D5B] text-white font-bold text-sm hover:bg-[#005548] transition-colors">
            Start Practicing →
          </button>
        </Link>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-4">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-[#006D5B] font-black text-sm">+26% this week ↑</p>
          </div>
            ...
            {['Week', 'Month'].map(r => (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-bold border transition-all ${
                  range === r ? 'bg-[#006D5B] text-white border-[#006D5B]' : 'border-gray-300 text-gray-700'
                }`}
              >
                {r}
              </button>
    ...
    <div>
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-4">
        <div className="flex items-center justify-between mb-2">
          <div>
            <p className="text-xs font-black text-gray-700 uppercase tracking-wider">Score Trend</p>
            <p className="text-[#006D5B] font-black text-sm">↑ 15 points across 3 tests</p>
          </div>
          <div className="flex gap-1">
            {['Week', 'Month'].map(r => (
              <button
        ...
        <LineChart points={chartData[range].data} labels={chartData[range].labels} colour="#006D5B" />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[
          ...
          <div key={s.label} className="bg-blue-100 border border-blue-300 rounded-xl p-3">
      ...
      <div className="bg-[#F0FAF8] border border-[#99D4CE] rounded-2xl p-4 mb-4">
        <p className="text-xs font-black text-[#006D5B] uppercase tracking-wider mb-3">Readiness Score</p>
        <div className="flex items-center gap-4">
          <div className="relative w-20 h-20 flex-shrink-0">
            <svg viewBox="0 0 80 80" className="w-full h-full -rotate-90">
              <circle cx="40" cy="40" r="32" fill="none" stroke="#CCE8E5" strokeWidth="10"/>
              <circle cx="40" cy="40" r="32" fill="none" stroke="#006D5B" strokeWidth="10"
                strokeDasharray={`${(67/90) * 2 * Math.PI * 32} ${2 * Math.PI * 32}`}
                strokeLinecap="round"/>
            </svg>
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-lg font-black text-[#005548]">67</span>
            </div>
          </div>
          <div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 pb-6">
        ...
        {[
          { rank: 1, name: 'Algebra',       pct: 38, topicId: 'math'      },
          { rank: 2, name: 'Combinations',  pct: 42, topicId: 'reasoning' },
          { rank: 3, name: 'Comprehension', pct: 51, topicId: 'english'   },
        ].map(f => (
          <div key={f.rank} className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-xs font-bold text-amber-600">{f.pct}%</span>
            <Link to={`/practice?topic=${f.topicId}`} className="ml-auto flex-shrink-0">
              <button className="text-[10px] font-black rounded-full px-2 py-0.5 text-gray-700 bg-gray-100 border border-gray-300 hover:bg-gray-200 transition-colors">
                Practice →
              </button>
            </Link>
          </div>
