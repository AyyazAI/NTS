...
function barColour(pct) {
  return pct >= 70 ? 'bg-teal-500' : pct >= 50 ? 'bg-amber-400' : 'bg-red-400'
}

function headerColour(pct) {
  return pct >= 70 ? 'text-teal-600' : pct >= 50 ? 'text-amber-600' : 'text-red-500'
}
...
function PracticeTab() {
  ...
  return (
    <div>
      ...
      {PRACTICE_TOPICS.map(t => <Accordion key={t.name} topic={t} />)}
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 mt-2">
        ...
        {WEAK_TODAY.map(w => (
          <div key={w.name} className="flex items-center justify-between mb-2.5">
            ...
            <Link to={`/practice?topic=${w.topic.toLowerCase()}`}>
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[
          ...
          { label: 'Best Score',      value: '67' },
          { label: 'Average Score',   value: '60' },
          ...
          <div className="relative w-20 h-20 flex-shrink-0">
            <svg viewBox="0 0 80 80" className="w-full h-full -rotate-90">
              ...
              <circle cx="40" cy="40" r="32" fill="none" stroke="#0D9488" strokeWidth="10"
                strokeDasharray={`${(67/100) * 2 * Math.PI * 32} ${2 * Math.PI * 32}`}
                strokeLinecap="round"/>
            </svg>
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col max-w-sm mx-auto">
      ...
      <main className="flex-1 px-4 pb-28 overflow-y-auto">
        ...
        {tab === 'practice' ? <PracticeTab /> : <MockTab />}
      </main>
